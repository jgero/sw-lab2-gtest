...
public:
  ...
  /* Der Kürze wegen sind die Basisdaten hier fest einprogrammiert.
     Normalerweise würden sie durch Einlesen einer Konfigurationdatei
  ...
  */
      : dieAbteilungen{{"Tischtennis", 5},
                       ...
                       {"Fitness", 10}},
                                               // aktiv, Grundbeitrag
        mitgliederstatus{{"Kind/Jugendlicher", {true, 7}},
                         {"Erwachsener", {true, 18}},
                         {"Ermaessigt", {true, 8}},
                         {"Passiv", {false, 20}}} {
        }
  ...
  std::size_t beitrag(const std::string& status,
    ...
    bool aktiv = mitgliederstatus.at(status).first;
    if(aktiv && abteilungen.size() == 0) {
      throw std::logic_error("Aktives Mitglied ohne Abteilungszuordnung!");
    }
    if(!aktiv && abteilungen.size() > 0) {
      throw std::logic_error("Passives Mitglied mit Abteilungszuordnung!");
    }
    // Preisberechnung
    auto preis = mitgliederstatus.at(status).second; // Grundpreis
